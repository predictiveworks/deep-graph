...
class Neighborhood[VD: ClassTag, ED: ClassTag]
  ...
  def setVertexMeasureCfg(value:VertexMeasureConfiguration[VD, ED]): Neighborhood[VD, ED] = {
  ...
  def transform(g:Graph[VD, ED])(implicit num:Numeric[ED]):DataFrame = {
     ...
     */
    val result:Graph[Double, ED] = NeighborhoodConnectivity.compute(g, vertexMeasureConfiguration)
    /*
     * The result of this method represents a VertexRDD
     * with (VertexId, Double (connectivity)
     */
