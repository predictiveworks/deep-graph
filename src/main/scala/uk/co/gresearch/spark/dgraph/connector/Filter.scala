...
case class Filters(
  promised: Set[Filter with Product with Serializable],
  optional: Set[Filter with Product with Serializable]) extends Set[Filter with Product with Serializable] {

  val filters: Set[Filter with Product with Serializable] = promised ++ optional

  override def iterator: Iterator[Filter with Product with Serializable] = filters.iterator

  override def contains(elem: Filter with Product with Serializable): Boolean = filters.contains(elem)

  override def +(elem: Filter with Product with Serializable): Set[Filter with Product with Serializable] = filters + elem
...
object Filters {
  def from(
            promised: Set[Filter with Product with Serializable],
            optional: Set[Filter with Product with Serializable] = Set.empty): Filters = Filters(promised, optional)

  def fromPromised(
                    promised: Filter with Product with Serializable,
                    morePromised: Set[Filter with Product with Serializable]): Filters = Filters(Set(promised) ++ morePromised, Set.empty)
  def fromOptional(
                    optional: Filter with Product with Serializable,
                    moreOptional: Set[Filter with Product with Serializable]): Filters = Filters(Set.empty, Set(optional) ++ moreOptional)
}
