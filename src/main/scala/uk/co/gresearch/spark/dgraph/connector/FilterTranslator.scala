...
case class FilterTranslator(columnInfo: ColumnInfo) {
   ...
   *
   * __KUP__
   * Replace Filter -> Filter with Product with Serializable
   *
   * @param filter spark filter
   * @return Some dgraph filters
   */
  def translate(filter: sql.sources.Filter): Option[Set[Filter with Product with Serializable]] = filter match {
    ...
    case In(ColumnName(column), values)
      if columnInfo.isObjectValueColumn(column) &&
        // check for non-null null-less non-empty values array
        Option(values).map(_.filter(Option(_).isDefined)).exists(_.length > 0) =>
      Some(Set(ObjectValueIsIn(values.toSet[Any])) ++ columnInfo.getObjectType(column)
        .map(t => Set(ObjectTypeIsIn(t))).getOrElse(Set.empty))
...
object FilterTranslator {
   ...
   * Replace Filter -> Filter with Product with Serializable
   */
  def simplify(filters: Set[Filter with Product with Serializable]): Set[Filter with Product with Serializable] = {
        ...
        .flatMap { case (cls, filters) =>
          cls match {
            case cls if cls == classOf[SubjectIsIn]
              => Set[Filter with Product with Serializable](filters.map(_.asInstanceOf[SubjectIsIn]).reduce(intersectSubjectIsIn))
            case cls if cls == classOf[IntersectPredicateNameIsIn]
              => Set[Filter with Product with Serializable](filters.map(_.asInstanceOf[IntersectPredicateNameIsIn]).reduce(intersectPredicateNameIsIn))
            case cls if cls == classOf[IntersectPredicateValueIsIn]
              => Set[Filter with Product with Serializable](filters.map(_.asInstanceOf[IntersectPredicateValueIsIn]).reduce(intersectPredicateValueIsIn))
            case cls if cls == classOf[SinglePredicateValueIsIn] =>
              filters
                .map(_.asInstanceOf[SinglePredicateValueIsIn])
                .groupBy(_.name)
                .mapValues(fs => Set[Filter with Product with Serializable](fs.reduce(intersectSinglePredicateValueIsIn)))
                .values.flatten
            case cls if cls == classOf[ObjectTypeIsIn]
              => Set[Filter with Product with Serializable](filters.map(_.asInstanceOf[ObjectTypeIsIn]).reduce(intersectObjectTypeIsIn))
            case cls if cls == classOf[ObjectValueIsIn]
              => Set[Filter with Product with Serializable](filters.map(_.asInstanceOf[ObjectValueIsIn]).reduce(intersectObjectValueIsIn))
            case _ => filters
          }
        }
        .toSet
    ...
    val predicateValues = trivialized.filter(_.isInstanceOf[PredicateValueIsIn]).map(_.asInstanceOf[PredicateValueIsIn].names)
      ...
      trivialized
        .flatMap {
          ...
          case PredicateNameIs(name) if singlePredicateName.isDefined && singleObjectValue.isDefined =>
            Some[Filter with Product with Serializable](SinglePredicateValueIsIn(name, singleObjectValue.get.values))
          ...
          case IntersectPredicateNameIsIn(_) if singlePredicateName.isDefined && singleObjectValue.isDefined =>
            Some[Filter with Product with Serializable](IntersectPredicateValueIsIn(singlePredicateName.get.names, singleObjectValue.get.values))
   ...
   */
  def simplify(filters: Filters, supported: Set[Filter with Product with Serializable] => Boolean): Filters = {
