...
case class PredicatePartitioner(schema: Schema,
  ...
  override def supportsFilters(filters: Set[connector.Filter with Product with Serializable]): Boolean = filters.map {
   ...
   * @return filters with ObjectTypeIsIn replaced
   */
  def replaceObjectTypeIsInFilter(filters: Set[Filter with Product with Serializable]): Set[Filter with Product with Serializable] =
    filters.map {
  ...
  def filter(clusterState: ClusterState, filters: Set[connector.Filter with Product with Serializable]): ClusterState =
    filters.foldLeft(clusterState)(filter)

  def filter(clusterState: ClusterState, filter: connector.Filter with Product with Serializable): ClusterState =
    filter match {
...
object PredicatePartitioner extends ClusterStateHelper {
   ...
   * @return operators
   */
  def getFilterOperators(filters: Set[Filter with Product with Serializable], predicates: Set[Predicate], properties: Set[String], edges: Set[String]): Set[Operator] = {
  ...
  def getPartitions(schema: Schema,
                    ...
                    filters: Set[Filter with Product with Serializable],
