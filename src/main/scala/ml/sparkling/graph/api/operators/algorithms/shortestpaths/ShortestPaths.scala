...
import ml.sparkling.graph.api.operators.algorithms.shortestpaths.ShortestPathsTypes.WithPathContainer
...
trait ShortestPaths {
   ...
   */
  def compute[VD, ED: ClassTag](graph: Graph[VD, ED], vertexPredicate: VertexPredicate[VD], treatAsUndirected: Boolean = false)(implicit num: Numeric[ED]): Graph[WithPathContainer, ED]
}
