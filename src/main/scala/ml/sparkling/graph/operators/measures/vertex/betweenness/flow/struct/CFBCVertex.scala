...
class CFBCVertex(
  ...
  lazy val relatedFlows: Array[CFBCFlow] = vertexFlows.filter(f => f.dst == id || f.src == id)
  lazy val availableSamples: Array[VertexId] = sampleVertices

  lazy val vertexPhi: Int = vertexFlows.count(_.src == id)

  lazy val flowsMap: Map[(VertexId, VertexId), CFBCFlow] = vertexFlows.map(f => ((f.src, f.dst), f)).toMap

  val (vertexFlows, neighboursFlows) = flows

  def isFinalized(k: Int): Boolean = sampleVertices.isEmpty || processedFlows >= k

  def getFlow(key: (VertexId, VertexId)): CFBCFlow = flowsMap.getOrElse(key, CFBCFlow.empty(key))

  def updateBC(currentFlowing: Double): CFBCVertex = {
  ...
  def updateBC(currentFlowing: Seq[Double]): CFBCVertex = {
  ...
  def removeFlows(toRemove: Seq[CFBCFlow]): CFBCVertex = {
