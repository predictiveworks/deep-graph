...
class NearlyOptimalBCProcessor[VD, ED: ClassTag](graph: Graph[VD, ED]) extends Serializable {
  ...
  val initVertexId: VertexId = graph.ops.pickRandomVertex()

  lazy val initGraph: Graph[NOVertex, ED] = initBFS
  ...
  def prepareVertices(startVertex: VertexId)(vertex: NOVertex): NOVertex = vertex.vertexId match {
  ...
  def applyMessages(round: Int)(vertexId: VertexId, vertex: NOVertex, messages: Option[List[NOMessage[VertexId]]]): NOVertex = {
  ...
  def updateSuccSet(vertex: NOVertex, pointer: Option[DFSPointer]): Option[Array[VertexId]] = pointer match {
    case Some(p) if p.next.nonEmpty && vertex.succ.nonEmpty =>
      Some(vertex.succ.getOrElse(empty[VertexId]).filterNot(it => p.next.contains(it)))
    case _ => vertex.succ
  }

  def updateBfsMap(vertexId: VertexId, map: Map[VertexId, NOBFSVertex], messages: List[NOMessage[VertexId]]): Map[VertexId, NOBFSVertex] = {
  ...
  private def sendPointer(triplet: EdgeTriplet[NOVertex, ED])(dst: VertexId, send: List[NOMessage[VertexId]] => Unit): Unit = {
  ...
  private def sendBFSExtendMessage(triplet: EdgeTriplet[NOVertex, ED])(dst: VertexId, send: List[NOMessage[VertexId]] => Unit): Unit = {
    val srcAttr = triplet.otherVertexAttr(dst)
  ...
  private def sendConfirmation(triplet: EdgeTriplet[NOVertex, ED])(dst: VertexId, send: List[NOMessage[VertexId]] => Unit): Unit = {
    val srcAttr = triplet.otherVertexAttr(dst)
    val dstAttr = triplet.vertexAttr(dst)
  ...
  private def sendAggregate(triplet: EdgeTriplet[NOVertex, ED])(dst: VertexId, send: List[NOMessage[VertexId]] => Unit): Unit = {
    val srcAttr = triplet.otherVertexAttr(dst)
    val dstAttr = triplet.vertexAttr(dst)
