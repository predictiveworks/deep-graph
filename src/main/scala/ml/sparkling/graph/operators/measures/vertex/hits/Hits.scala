...
object Hits extends VertexMeasure[(Double, Double)] {
   ...
   */
  def computeBasic[VD: ClassTag, ED: ClassTag](graph: Graph[VD, ED], continuePredicate: ContinuePredicate = convergenceAndIterationPredicate(1e-8), normalize: Boolean = true): Graph[(Double, Double), ED] = {
    var iteration = 0
