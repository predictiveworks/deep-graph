...
class NOVertex(val vertexId: VertexId,
  ...
  def setParent(idParent: VertexId): NOVertex = NOVertex(vertexId, bfsMap, Some(idParent), succ, dfsPointer, bc)

  def setPredecessorAndSuccessors(newPred: Option[VertexId], newSucc: Option[Array[VertexId]]): NOVertex =
    NOVertex(vertexId, bfsMap, newPred, newSucc, dfsPointer, bc)

  val isCompleted: Boolean = pred.nonEmpty && succ.nonEmpty

  val leaf: Boolean = succ.isEmpty

  lazy val bfsRoot: Boolean = bfsMap.contains(vertexId)

  lazy val lowestSucc: Option[VertexId] = succ.getOrElse(Array.empty).sorted.headOption

  lazy val eccentricity: Double = if (bfsMap.isEmpty) 0 else bfsMap.map({ case (id, v) => v.distance}).max

  def withDfsPointer(pointer: Option[DFSPointer]): NOVertex =
    NOVertex(vertexId, bfsMap, pred, succ, pointer, bc)

  def update(bfsMap: Map[VertexId, NOBFSVertex] = bfsMap, succ: Option[Array[VertexId]] = succ, dfsPointer: Option[DFSPointer] = dfsPointer, bcInc: Double = 0): NOVertex =
