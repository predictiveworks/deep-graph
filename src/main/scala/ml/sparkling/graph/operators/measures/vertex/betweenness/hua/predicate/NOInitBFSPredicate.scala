...
class NOInitBFSPredicate extends BFSPredicate[NOVertex, List[NOMessage[VertexId]]] {
  ...
  def updateVertex(vertex: NOVertex, messages: List[NOMessage[VertexId]]): NOVertex = {
    val parent = extractParent(vertex, messages)
    val succ = extractSuccessors(vertex, messages)
    vertex.setPredecessorAndSuccessors(parent, succ)
  }

  def extractParent(vertex: NOVertex, messages: List[NOMessage[VertexId]]): Option[VertexId] = {
    vertex.pred match {
  ...
  def extractSuccessors(vertex: NOVertex, messages: List[NOMessage[VertexId]]): Option[Array[VertexId]] =
    vertex.succ match {
