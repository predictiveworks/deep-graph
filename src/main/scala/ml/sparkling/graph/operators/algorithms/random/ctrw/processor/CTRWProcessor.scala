...
class CTRWProcessor[VD, ED: ClassTag](graph: Graph[VD, ED], factory: MessageFactory) extends Serializable {
  ...
  lazy val initGraph: Graph[CTRWVertex, ED] = prepareRawGraph
  ...
  def createInitMessages(sampleSize: Int)(vertex: CTRWVertex): CTRWVertex = {
    val msg = for (i <- 0 until sampleSize) yield factory.create(vertex)
    CTRWVertex(vertex.id, vertex.neighbours, msg.toArray)
  }

  def sendMessage(triplet: EdgeTriplet[CTRWVertex, ED]): Iterator[(VertexId, List[CTRWMessage])] = {
  ...
  def sendMessageCtx(round: Int)(edgeContext: EdgeContext[CTRWVertex, _, List[CTRWMessage]]): Unit = {
    ...
    def send(msg: List[CTRWMessage], f: List[CTRWMessage] => Unit): Unit =
  ...
  def mergeMessages(msg1: List[CTRWMessage], msg2: List[CTRWMessage]): List[CTRWMessage] = msg1 ++ msg2

  def applyMessages(round: Int)(vertexId: VertexId, data: CTRWVertex, messagesOps: Option[List[CTRWMessage]]): CTRWVertex = {
