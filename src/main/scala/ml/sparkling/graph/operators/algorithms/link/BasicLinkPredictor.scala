...
object BasicLinkPredictor extends MeasureBasedLnkPredictor {
  ...
  override def predictLinks[V: ClassTag, E: ClassTag, EV: ClassTag, EO: ClassTag](graph: Graph[V, E],
                                                                                  ...
                                                                                  threshold: EO,
                                                                                  treatAsUndirected:Boolean=false)(implicit num: Numeric[EO]): RDD[(VertexId, VertexId)] = {
    ...
    val existingEdgesTuples=graph.edges.map(e=>((e.srcId,e.dstId),0))
    val newEdges=edgesAboveThreshold.leftOuterJoin(existingEdgesTuples).filter{
