...
case object ToNewId{
  def apply(id:VertexId,vertexWrapper: VertexWrapper): VertexId =vertexWrapper.currentId
}
...
case object ChangedStatsAgregator{
  def add(b1:ChangedStats,b2:VertexWrapper): ChangedStats =if(b2.changed) ChangedStats(b1.changed+1,b1.notChanged) else ChangedStats(b1.changed,b1.notChanged+1)
  def add(b1:ChangedStats,b2:ChangedStats): ChangedStats =b1+b2
}


case object LPCoarsening extends CoarseningAlgorithm{
  val logger: Logger =Logger.getLogger(LPCoarsening.getClass)
  override def coarse[VD:ClassTag,ED:ClassTag](graph: Graph[VD, ED],treatAsUndirected:Boolean=false,checkpointingFrequency:Int=50,edgeValueSelector:EdgeValueSelector=DefaultEdgeValueSelector): Graph[Component, ED] = {
    ...
    var iterationGraph: Graph[VertexWrapper, ED] =filteredGraph.mapVertices((vId, _)=>VertexWrapper(vId,vId,vId))
    ...
    while(changed){
      ...
      val innerIterationGraph=iterationGraph.pregel[List[VertexId]](List.empty,maxIterations = 1)(
        vprog=(vId,data,msg)=>{
          if (msg.isEmpty) VertexWrapper(data.currentId,data.beforeCurrentId,data.beforeBeforeCurrent) else
          ...
          }.getOrElse(VertexWrapper(data.currentId,data.beforeCurrentId,data.beforeBeforeCurrent))
        },
        sendMsg = triplet=>{
     ...
     val innerIterationGraphSemi=innerIterationGraph.pregel[List[VertexId]](List.empty,maxIterations = 1)(
        vprog=(vId,data,msg)=>{
          if(msg.isEmpty) data else
          msg.groupBy(id=>id).mapValues(_.length).get(data.currentId).map(count => data).getOrElse({
            if(data.currentId==data.beforeBeforeCurrent){
              VertexWrapper(vId,data.currentId,data.beforeCurrentId)
            }  else
