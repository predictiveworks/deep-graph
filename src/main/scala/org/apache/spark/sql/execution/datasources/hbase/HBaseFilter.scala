...
object TypedFilter {
  ...
  def empty: TypedFilter = TypedFilter(None, FilterType.Und)
...
object HRF {
  def empty[T]: HRF[T] = HRF[T](Array(ScanRange.empty[T]), TypedFilter.empty)
}
    ...
    def Greater(attribute: String, value: Any): HRF[Array[Byte]]  = {
        ...
        bound => {
          if (relation.singleKey) {
            HRF(bound.greater.map(x => ScanRange(Some(Bound(x.low, inc = true)),
              Some(Bound(x.upper, inc = true)))), TypedFilter.empty)
          } else {
            val s = bound.greater.map( x=>
              ScanRange(relation.rows.length,
                x.low, lowInc = true, x.upper, upInc = true, relation.getField(attribute).start))
        ...
        bound => {
          if (relation.singleKey) {
            HRF(bound.less.map(x =>
              ScanRange(Some(Bound(x.low, inc = true)),
                Some(Bound(x.upper, inc = true)))), TypedFilter.empty)
          } else {
    ...
    val f = filter match {
      ...
      case EqualTo(attribute, value) =>
        process(value, relation, attribute,
          bound => {
            if (relation.singleKey) {
              HRF(Array(ScanRange(Some(Bound(bound.value, inc = true)),
                Some(Bound(bound.value, inc = true)))), TypedFilter.empty, handled = true)
            } else {
              val s = ScanRange(relation.rows.length,
                bound.value, lowInc = true, bound.value, upInc = true, relation.getField(attribute).start)
          ...
          bound => {
            val s = ScanRange(relation.rows.length,
              bound.value, true, bound.value, upInc = true, relation.getField(attribute).start)
            HRF(Array(s), TypedFilter.empty)
          })
