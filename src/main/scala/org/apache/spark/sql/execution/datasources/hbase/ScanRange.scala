...
object ScanRange {
  ...
  def empty[T]: ScanRange[T] = ScanRange[T](None, None)

  // split (a, b] to (a, b) and [b, b]
  def split[T](range: ScanRange[T])(implicit ordering: Ordering[T]): Seq[ScanRange[T]] = {
    if (range.end.exists(_.inc) &&
      compare(range.start, range.end, ordering) != 0
    ) {
      Seq(ScanRange(range.start, Some(Bound(range.end.get.point, inc = false))),
        ScanRange(
          Some(Bound(range.end.get.point, inc = true)),
          Some(Bound(range.end.get.point, inc = true))))
    } else {
      Seq(range)
    }
  ...
  def and[T](
    ...
    ranges.foreach { range =>
      val l =  {
        if (compare(bounds.start, range.start, ordering) >= 0) {
          bounds.start
        } else {
          range.start
        }
      ...
      val r = {
        if (compare(bounds.end, range.end, ordering, min = false) <= 0) {
          bounds.end
        } else {
          range.end
        }
  ...
  def or[T](
    ...
    val lIdx = bSearch(l, extra.start, ordering, min = true)
    val uIdx = bSearch(u, extra.end, ordering, min = false)
    val (low, lowIdx) = {
    ...
    in match {
      ...
      case _: Array[Byte] | _: Byte | _: String | _: UTF8String =>
        Some(BoundRanges(
          ...
          /*
           * Modification made by Dr. Stefan Krusche, ByteMax must
           * be used as an instance of [Byte].
           */
          Array(BoundRange(b, Array.fill(b.length)(ByteMax.asInstanceOf[Byte]))), b))
