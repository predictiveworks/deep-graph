...
final class SHCCredentialsManager private() extends Logging {
  ...
  private val credentialsManagerEnabled = {
    val isEnabled = sparkConf.getBoolean(SparkHBaseConf.credentialsManagerEnabled, defaultValue = false) &&
      UserGroupInformation.isSecurityEnabled
    logInfo(s"SHCCredentialsManager was${if (isEnabled) "" else " not"} enabled.")
    isEnabled
  }

  val tokenUpdateExecutor: ScheduledExecutorService = Executors.newSingleThreadScheduledExecutor(
