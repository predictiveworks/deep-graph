 ...
 *
 * Modification made by Dr. Krusche & Partner PartG are also licensed under
 * the Apache Software License, Version 2.0.
 *
 */
...
object SparkHBaseConf {
  ...
  // in milliseconds
  val connectionCloseDelay: Int = 10 * 60 * 1000
   ...
   *
   * The HBase datasource in the context of this project is
   * exclusively used to support HGraphDB read & write requests.
   *
   * Therefore, dependencies to Avro and Phoenix in Hortonworks
   * implementation are removed and the reference to HGraphDB
   * is added.
   */

  // val Avro: String = classOf[Avro].getSimpleName
  // val Phoenix: String = classOf[Phoenix].getSimpleName

  val HGraphDB: String = classOf[HGraphDB].getSimpleName
  val PrimitiveType: String = classOf[PrimitiveType].getSimpleName
