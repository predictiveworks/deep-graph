...
object SHCDataTypeFactory {
  ...
  def create(f: Field): SHCDataType = {
    ...
    /**
     * Modifications by Dr. Krusche & Partner PartG: Dependencies
     * to [Avro] and [Phoenix] were removed and support [HGraphDB]
     * added.
     */
    if (f.fCoder == SparkHBaseConf.HGraphDB) {
      new HGraphDB(Some(f))
//    }
...
//      new Phoenix(Some(f))
  ...
  def create(coder: String): SHCDataType = {
     ...
     * added.
     */
    if (coder == SparkHBaseConf.HGraphDB) {
      new HGraphDB()
